...
$(document).ready(function() {
	...
	$(".main_mnu .drop a").first().click(function(e){
		if($(this).parent().hasClass("drop_on")){
			...
			$(".main_mnu .drop a").first().parent().removeClass("drop_on");
		}else{
			setTimeout(function () { $(".main_mnu .drop a").first().next().slideDown("slow"); }, 180);
	...
	$(document).click(function(event) {
		if ($(".drop_menu").is(":visible") ){
			if ($(event.target).closest(".drop_menu ul, .drop_s, .drop_sub").length) return;
			$(".drop_menu").slideUp("slow");
			$(".drop_menu").children().children(".drop").children("a").removeClass("dm_act").next().fadeOut();
			$(".drop_s").children().children(".drop").children("a").removeClass("ds_act").next().fadeOut();
			$(".drop_sub").children().children(".drop").children("a").removeClass("dsub_act").next().fadeOut();
	...
	$(".sheme_prod_filter .active a").click(function(e){
		$(".sheme_prod_filter li").show();
		e.preventDefault();
	});
	$(document).click(function(event) {
		if ($(".sheme_prod_filter li").is(":visible") ){
			if ($(event.target).closest(".sheme_prod_filter").length) return;
			$(".sheme_prod_filter li").hide();
			event.stopPropagation();
		}
	});
