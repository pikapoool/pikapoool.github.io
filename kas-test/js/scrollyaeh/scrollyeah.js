...
(function($){
  ...
  function getTranslatePercent(pos) {
    return {left: pos+"%"};
  }
  ...
  function doScrollyeah(scrollyeah, o) {
	...
	function eqTo60(sPos, sMPos){
		return sPos/(sMPos/60);
	}

    function animate(block, pos, time, overPos, extra, isPercent) {
      var POS = isNaN(pos) ? 0 : pos;
	  if(isPercent){
		  POS = eqTo60(POS, shaftMinPos);
	  }
	  
      clearBackAnimate(block);
      if (overPos) {
		    if(isPercent){
				  overPos = eqTo60(overPos, shaftMinPos);
			}
        POS = overPos;
        block.data({
          backAnimate: setTimeout(function(){
            animate(block, pos, Math.max(o__transitionDuration, time/2), false, extra, isPercent);
          }, time)
        });
      }
	  if(isPercent){
		  block.stop().animate(getTranslatePercent(POS), time, o__bez);
	  }
	  else{
    ...
    function onResize() {
	  ...
	  //---------------------------------------------------------------------------------------------------------
	  //Доработка скролла для слайдера
	  $("#sldr_hrzntl_scrll").attr("max", shaftMinPos);
	  /*if(!shaftMinPos){
...
//      }
    ...
    function touch(mouseDown, mouseMove, mouseUp) {
      ...
      function onMouseMove(e) {
		...
		//-----------------------------------------------------------------------------------------------------
		//Допиливание скролла
		var move_val_percent = shaftPos/(shaftMinPos/60);
		$(".horisontal_scroll_for_slider .rangeslider--horizontal .rangeslider__handle").css("left", move_val_percent+"%");
		//-----------------------------------------------------------------------------------------------------
      }
    ...
    function shaftOnMouseUp(dirtyLeft, timeDiff, isSwipe, timeFromLast, sameDirection, posDiff, e) {
      ...
      if (dirtyLeft > shaftMaxPos) {
		...
		//-----------------------------------------------------------------------------------------------------
			//Допиливание скролла
			var range_slider_handle = $(".horisontal_scroll_for_slider .rangeslider--horizontal .rangeslider__handle");
			animate(range_slider_handle, newPos, time, overPos, false, true);
		  //-----------------------------------------------------------------------------------------------------
      }
